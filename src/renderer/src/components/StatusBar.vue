...
const collabLabel = computed(() => {
  ...
  const role = collab.session.role === 'host' ? '호스트' : '참가'
  const live = collab.connected ? '' : ' (끊김)'
  return `협업 ${role} · ${collab.viewerCount}명${live}`
})

function openCollab(): void {
  if (collab.session?.role === 'host') collab.openDialog()
  else collab.openJoinDialog()
}
</script>
...
<template>
  <footer class="status">
    ...
    <button v-if="collabLabel" class="status-link" type="button" @click="openCollab">{{ collabLabel }}</button>
